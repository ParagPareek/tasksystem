...
const Display = () => {
  ...
  const fetchLists = async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:3030/lists/${userId}`);
  ...
  const fetchTasks = async (listId) => {
    try {
      const response = await axios.get(`http://localhost:3030/tasks/${listId}`);
      setLists((prevLists) =>
        prevLists.map((list) =>
  ...
  const addList = async () => {
    ...
    try {
      await axios.post("http://localhost:3030/lists/create", {
        name: listName,
        user: userId,
      });
      setListName("");
  ...
  const addTask = async (list) => {
    ...
    try {
      await axios.post("http://localhost:3030/tasks/create", {
        name: taskName,
        listId: list._id,
        listname: list.name,
        userId,
  ...
  const updateListColor = async (listId, newColor) => {
    try {
      await axios.put("http://localhost:3030/lists/updatecolor", {
        listId,
        color: newColor,
      });
      setLists((prevLists) =>
  ...
  const delist = async (listId) => {
    try {
      await axios.delete("http://localhost:3030/lists/delete", {
        data: { listId },
      });
  ...
  const onDragEnd = async (result) => {
    ...
    try {
      await axios.put(
        `http://localhost:3030/tasks/update/${draggableId}`,
        {
